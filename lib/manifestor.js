 ...
 */
var build = function(path, externalFiles) {
  return util.getFilesFromDisk(path)
    .then(function(files) {
...
function getConfig(context) {
  return fs.readFileAsync(defaults.CONFIG_NAME)
    ...
    .then(function(parsed){
      ...
      if (context) {
        if (parsed[context]) {
          config = parsed[context]
        } else {
          console.log("Context not found in configuration file".red)
          throw "Context not found in configuration file"
        }
      } else {
        config = parsed
      }
